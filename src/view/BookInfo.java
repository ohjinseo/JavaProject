...
public class BookInfo extends JFrame {
	 ...
	 * Create the frame.
	 */
	public BookInfo(String book_ISBN) {
		...
		try { // DB 접근
			ResultSet rs = dbConn.executeQuery("SELECT BOOK_TITLE, BOOK_AUTHOR, BOOK_PUB, BOOK_PRICE, BOOK_ISBN, BOOK_LINK, BOOK_DESCRIPTION, BOOK_IMAGE\r\n"
					+ "FROM BOOK\r\n"
					+ "WHERE BOOK_ISBN = '"+book_ISBN+"' AND BOOK_PRE = TRUE;"); // DB에서 ISBN으로 책 정보 검색
			while(rs.next()){
				...
				try {
					...
					Image resize_img = img.getScaledInstance(170, 170, Image.SCALE_SMOOTH); // 이미지 크기 170x170로 크기 조절하여 resize_img에 저장
